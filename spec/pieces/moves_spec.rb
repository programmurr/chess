...
describe Moves do
  context '#valid_move?' do
    ...
    xit 'returns true if the WhiteKnight can move to an empty square' do
      player = double('Player', move: %w[g1 f3], piece: WhitePiece.new)
      move = Moves.new(player, @board)
      expect(move.valid_move?).to eq true
    end

    xit 'returns false if WhiteKnight tries to move to a square occupied by the same piece color' do
      player = double('Player', move: %w[g1 e2], piece: WhitePiece.new)
      move = Moves.new(player, @board)
      expect(move.valid_move?).to eq false
    end

    xit 'returns false if WhiteKnight tries to move to a square not in its move range' do
      player = double('Player', move: %w[g1 g2], piece: WhitePiece.new)
      move = Moves.new(player, @board)
      expect(move.valid_move?).to eq false
    end

    xit 'returns true if the WhiteRook can move to an empty square' do
    ...
    xit 'returns false if WhiteRook tries to move to a square occupied by the same piece color' do
    ...
    xit 'returns false if WhiteRook tries to move in an invalid direction' do
    ...
    xit 'returns true if the WhiteBishop can move to an empty square' do
    ...
    xit 'returns false if WhiteBishop tries to move to a square occupied by the same piece color' do
    ...
    xit 'returns false if WhiteBishop tries to move in an invalid direction' do
